...
insert into Cliente (id, CPF, nome, telefone, email, endereco) values
...
insert into Veiculo (id, modelo, marca, ano, placa, valorDiaria, estado) values
...
insert into Manutencao (id, idVeiculo, descricao, dataManutencao, custo) values
...
insert into Pagamento (id, forma, dataPagamento, valorTotal, estado) values
...
insert into Locacao (id, idCliente, idPagamento, dataInicio, dataFim) values
...
SELECT SUM(valorTotal) AS Total_Arrecadado FROM pagamento
WHERE estado = 'Pago';

SELECT v.modelo, v.marca, COUNT(lv.idLocacao) AS total_locacoes
FROM veiculo v
JOIN locacaoVeiculo lv ON v.id = lv.idVeiculo
GROUP BY v.id, v.modelo, v.marca ORDER BY total_locacoes DESC;

SELECT c.nome, SUM(p.valorTotal) AS valor_pendente
FROM cliente c 
JOIN locacao l ON c.id = l.idCliente
JOIN pagamento p ON l.idPagamento = p.id
